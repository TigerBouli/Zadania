...
class KategorieTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        performSegue(withIdentifier: "doZadan", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let docelowyVC = segue.destination as! ZadaniaViewController
        
        if let indexPath = tableView.indexPathForSelectedRow {
            docelowyVC.wybranaKategoria = kategorie[indexPath.row]
        }
    }
